...
namespace Reports.Ventas
{
    public class VentaReports : IDocument
    {
        private readonly List<VentaListadoReporteDTO> _data;
        private readonly bool _mostrarDetalle;

        public VentaReports(List<VentaListadoReporteDTO> data, bool mostrarDetalle = false)
        {
            _data = data;
        ...
        public void Compose(IDocumentContainer container)
        {
            try
            {
                var totalGeneral = _data.Sum(x => x.totalventa);
                ...
                container.Page(page =>
                {
                    ...
                    {
                        foreach (var venta in _data)
                        {
                            col.Item().PaddingTop(10).Background(Colors.Grey.Lighten4).Padding(6).Row(r =>
                            {
                                r.RelativeItem().Column(c =>
                                {
                                    c.Item().Text($"{venta.numtipocomprobante}-{venta.numventa}")
                                        .Bold().FontSize(11);
                                    c.Item().Text($"{venta.cliente} | {venta.nrodoc}")
                                        .FontSize(9).FontColor(Colors.Grey.Darken2);
                                });

                                r.ConstantItem(150).AlignRight().Column(c =>
                                {
                                    c.Item().Text($"Fecha: {venta.fechaventa:dd/MM/yyyy}").FontSize(9);
                                    c.Item().Text($"Condición: {venta.condicionpago}").FontSize(9);
                                });
                            });

                            if (_mostrarDetalle && venta.detalle != null && venta.detalle.Count > 0)
                            {
                            ...
                            col.Item().AlignRight().Width(180).PaddingVertical(5).Table(t =>
                            {
                                ...
                                t.Cell().AlignRight().Text(venta.totaliva.ToString("N0")).FontSize(8).FontColor(Colors.Grey.Darken1);

                                t.Cell().Text("TOTAL:").FontSize(9).Bold();
                                t.Cell().AlignRight().Text(venta.totalventa.ToString("N0")).FontSize(9).Bold().FontColor("#1A3B5D");
                            });
            ...
            {
                throw new Exception("Error generando el PDF de ventas.", ex);
            }
        }

        private object ToString(string v)
        {
